...
static constexpr double REFLE_GRASS = 0.10;
static constexpr double REFLE_CONCR = 0.50;
static constexpr double GRAV = 9.81;
static constexpr double SIGMA = 5.67037e-8;
static constexpr double GAMMA = 66.0;
static constexpr double EMISS = 0.98;
static constexpr double MU_ST = 1.827e-5;
static constexpr double TR0 = 527.0;
static constexpr double CCONV1 = 120.0;
static constexpr double KJDAY_TO_W = 1000.0 / (3600.0 * 24.0);
static constexpr double KTO_R = 9.0 / 5.0;
static constexpr double SCHMIDT = 0.61;
static constexpr double COAT_CONST = 1.90e-5;
static constexpr double ZC = 11000.0;
static constexpr double TBODY_C = 39.0;
static constexpr double LASMIN = 10.0;
static constexpr double RESPINCR = 7.64;
static constexpr double RAINFRAC = 0.3;
...
HeatPair evaluate_at_temperature(const HallConfig& cfg,
    ...
    const std::array<double, 9> LIBRARY10{0.6, 0.012, 1.00, 64.1, 3.08, 1.73, 1.000, 35.3, 1.00};
    const std::array<double, 9> LIBRARY20{0.6, 0.012, 1.12, 64.1, 4.89, 0.80, 1.30, 34.5, 0.91};
    const std::array<double, 9> LIBRARY30{0.56, 0.012, 1.09, 64.1, 4.44, 1.03, 1.000, 34.7, 0.93};
    const auto& LIBRARY = (breed_library == 1 ? LIBRARY10 : (breed_library == 2 ? LIBRARY20 : LIBRARY30));

    const double REFLC = LIBRARY[0];
    const double LC = LIBRARY[1];
    const double AREAFACTOR = LIBRARY[2];
    const double CBSMAX = LIBRARY[3];
    const double RBCSf = LIBRARY[6];
    ...
    const double AREA = 0.14 * std::pow(tbw, 0.57) * AREAFACTOR;
    ...
    double panting = 0.25;
    double irv = brv + panting * ((RESPINCR - 1.0) * brv);
    ...
    double NERESPWM = 1.1 * std::pow(RESPINCR * brr, 2.78) * 1e-5 * panting;
    ...
    const double DLC = (COAT_CONST * wind) / (((COAT_CONST * wind) / LC) + 1.0 / (ZC * LC));
    const double DIFFC = 0.187e-9 * std::pow(TAVGK, 2.072);
    double rainReduction = 1.0 - std::min(RAINFRAC, rain * RAINFRAC / 24.0);
    if (rainReduction <= 1e-9) rainReduction = 1e-9;
    const double CSC = (1.0 / (ZC * (LC - DLC) * (0.078 / 100.0))) / rainReduction;
    ...
    const double VISCAIR = MU_ST * (((0.555 * TR0 + CCONV1) / (0.555 * TAVGR + CCONV1)) * std::pow(TAVGR / TR0, 1.5));
    ...
    double REFLE = 0.0;
    if (housing_mode == 1.0) REFLE = REFLE_GRASS;
    else if (housing_mode == 2.0) REFLE = REFLE_CONCR;
    ...
    auto coat_conv = [&](double TcoatC) {
        ...
        const double GRASHOF = (GRAV * std::pow(DIAMETER, 3.0) * P / 100.0 * (TcoatC - air_temp_c) +
                                SCHMIDT * (Ec * TcoatC - Ea * air_temp_c)) /
    ...
    double TskinC_max = TBODY_C;
    for (int iter = 0; iter < 50000; ++iter) {
        const double MetheatSKIN = METABFEED - TNRESP;
        const double TskinC = TBODY_C - MetheatSKIN / CONDBS;
        const double LASMAXPHYS = LASMIN + LIBRARY[4] * std::exp(LIBRARY[5] * (TskinC - LIBRARY[7])) * L / 3600.0;
        ...
        const double LASMAXENV = (RHOAIR * CP * 1000.0 / GAMMA) * (VPSKINTOT - VPAIRTOT) / RV;
        const double LASMAXCORR = std::min(LASMAXPHYS, LASMAXENV);
        const double ACTSW = LASMIN + 1.0 * (LASMAXCORR - LASMIN);
        const double MetheatCOAT = MetheatSKIN - ACTSW;
        const double TcoatC = TskinC - MetheatCOAT / CSC;
        const double TcoatK = TcoatC + CTOK;
        const double LB = EMISS * SIGMA * std::pow(TcoatK, 4.0);
        const double LWRCOAT = (EMISS * ((LWRSKY + LWRENV) / 2.0) - LB) / rainReduction;
    ...
    double TskinC_min = TBODY_C;
    for (int iter = 0; iter < 50000; ++iter) {
        const double MetheatSKIN = METABFEEDC - TNRESP;
        const double TskinC = TBODY_C - MetheatSKIN / CONDBS;
        const double ACTSW = LASMIN;
        const double MetheatCOAT = MetheatSKIN - ACTSW;
        const double TcoatC = TskinC - MetheatCOAT / CSC;
        const double TcoatK = TcoatC + CTOK;
        const double LB = SIGMA * std::pow(TcoatK, 4.0);
        const double LWRCOAT = (EMISS * ((LWRSKY + LWRENV) / 2.0) - EMISS * LB) / rainReduction;
        const double CONVCOAT = coat_conv(TcoatC);
        const double MetheatAIR = (MetheatCOAT + SWR - RAINEVAP + LWRCOAT - CONVCOAT);
...
ThermoregulationState evaluate_thermoregulation(const HallConfig& cfg,
    ...
    const HeatPair current = evaluate_at_temperature(cfg, ambient_temp_c, relative_humidity_pct, wind_m_s, rad_kj_m2_day,
                                                     cloud_okta, rain_mm_day, aha, housing_mode, breed_library, effective_weight, out.heat_production_multiplier);
    ...
    const double METTBWACT = std::pow(std::max(50.0, effective_weight) * 0.9, 0.75);
    const double MAINTME = 311.0 * METTBWACT;
    const double TOTNE = MAINTME + MAINTME * (out.heat_production_multiplier - 1.0);
    const double HIF = 0.30;
    const std::array<double, 9> LIBRARY30{0.56, 0.012, 1.09, 64.1, 4.44, 1.03, 1.000, 34.7, 0.93};
    const double AREAFACTOR = LIBRARY30[2];
    const double area = 0.14 * std::pow(std::max(50.0, effective_weight), 0.57) * AREAFACTOR;
    out.animal_heat_threshold_w_m2 = TOTNE * (1.0 + HIF / (1.0 - HIF)) / area * 1000.0 / (3600.0 * 24.0);
...
ThermoregulationState evaluate_heterogeneous_thermoregulation(const HallConfig& cfg,
    ...
    for (const auto& c : herd.cohorts) {
        auto t = evaluate_thermoregulation(cfg, ambient_temp_c, relative_humidity_pct, wind_m_s, rad_kj_m2_day, cloud_okta, rain_mm_day, aha, housing_mode, c.breed_library, c.avg_weight_kg, c.avg_heat_multiplier);
